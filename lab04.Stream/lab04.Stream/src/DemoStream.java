...
public class DemoStream {
  ...
  public static void map01() {
    ...
    var result = manNames.stream().map(element -> element.toUpperCase()) // Ánh xạ từng phần tự sang dạng UpperCase
  ...
  public static void map02() {
    ...
    var result = manNames.stream().map(String::toUpperCase) // Ánh xạ từng phần tự sang dạng UpperCase
        .collect(Collectors.toList()); // Gom nhặt phần tử, chuyển sang cấu trúc List
  ...
  public static void map03() {
    ...
    manNames.stream().map(String::toUpperCase).forEach(System.out::println);
  }

  public static void filter01() {
    ArrayList<Integer> manNames = new ArrayList<>(List.of(1, 3, 2, 4, 5, 6, 9, 2));

    manNames.stream().filter(p -> p % 2 == 0) // Lọc chỉ lấy số chẵn
  ...
  public static void reduce01() {
    ...
    Integer result = manNames.stream().reduce(0, (a, b) -> a + b); // gập các phần tử lại để tính tổng
  ...
  public static void summingInt() {
    ...
    Integer result = manNames.stream().collect(Collectors.summingInt(Integer::intValue));
  ...
  public static void maxBy() {
    ...
    Optional<Integer> result = manNames.stream().collect(Collectors.maxBy(Integer::compareTo));
  ...
  public static void minBy() {
    ...
    Optional<Integer> result = manNames.stream().collect(Collectors.minBy(Integer::compareTo));
  ...
  public static void average() {
    ...
    Double result = manNames.stream().collect(Collectors.averagingInt(Integer::intValue)); // Tính trung bình
  ...
  public static void distinct() {
    ...
    manNames.stream().distinct().forEach(System.out::println);
  ...
  public static void sortByName02() {
    people.stream().sorted(Comparator.comparing(Person::getName).reversed()) // Sắp xếp giảm Z-A
        .forEach(System.out::println);
  }

  public static void filterOutChinese() {
    people.stream().filter(p -> !p.getNationality().equals("China")).sorted(Comparator.comparing(Person::getName))
        .forEach(System.out::println);
  }
  ...
  public static void peek() {
    /*
     * people .stream() .peek(Person::increaseAge) .collect(Collectors.toList());
     */

    people.stream().forEach(Person::increaseAge); // cách này tốt hơn
  ...
  public static List<Person> getByName(int x, int y) {
    var result = people.stream().filter(p -> (p.getAge() > x && p.getAge() < y)).collect(Collectors.toList());
    System.out.println(result);
  ...
  public static void getAverageAge() {
    var averageAge = people.stream().collect(Collectors.averagingDouble(Person::getAge));
