...
public class PersonController {
    ...
    public String listAll(Model model,
                          @CookieValue(value = "addupdate", defaultValue = "Failed") String addupdate) {
        model.addAttribute("people", personDao.getAll());
        model.addAttribute("addupdate", addupdate);
        return "allpeople";
    }
     ...
     public String save(@Valid @ModelAttribute Person person, BindingResult result,
                        RedirectAttributes redirect, HttpServletResponse res) {
        ...
        if (person.getPhoto().getOriginalFilename().isEmpty()){
         ...
         Cookie cookie = new Cookie("addupdate", "successfully");
         cookie.setMaxAge(60);
